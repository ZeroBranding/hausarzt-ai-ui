...
const AuthStep = ({ onComplete, onBack }: AuthStepProps) => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      await requestJson<any>(`/api/auth/register`, {
        method: 'POST',
        body: JSON.stringify({
          email: registerEmail,
      ...
      await requestJson<any>(`/api/auth/send-verification`, {
        method: 'POST',
      ...
      setError(err instanceof Error ? err.message : 'Verbindungsfehler. Bitte versuchen Sie es erneut.');
    }
    setIsLoading(false);
  };
  ...
  const checkVerification = async () => {
    ...
    setError(null);
    try {
      const data = await requestJson<any>(`/api/auth/check-verification?email=${encodeURIComponent(verificationEmail)}`);
      if (data.verified) {
        setIsVerified(true);
        handleAutoLogin();
      } else {
  ...
  const handleGoogleAuth = async (isRegister: boolean = false) => {
    ...
    try {
      ...
      const data = await requestJson<any>(`/api/auth/oauth/google?redirect_uri=${redirectUri}&state=${oauthState}`);
  ...
  const handleAppleAuth = async (isRegister: boolean = false) => {
    ...
    try {
      const redirectUri = encodeURIComponent(window.location.origin + '/oauth/callback');
      const oauthState = isRegister ? 'register' : 'login';
      const data = await requestJson<any>(`/api/auth/oauth/apple?redirect_uri=${redirectUri}&state=${oauthState}`);
      sessionStorage.setItem('booking_state', JSON.stringify({
        selectedDate: state.selectedDate,
        selectedTime: state.selectedTime,
      }));
